...
int			maps[1001][1001], visit[1001][1001][2], n, m, dx[4] = {1, -1, 0, 0}, dy[4] = {0, 0, 1, -1};
queue<pair<pair<int, int>, pair<int, int> > >		q;		// <<cnt, break>, <r, c>>

int		is_valid(int r, int c, int wall, int *ref)
{
	if (r < 1 || r > n) return (0);
	if (c < 1 || c > m) return (0);
	if (maps[r][c] == 1)
...
int		main(void)
{
	...
	while (!q.empty())
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (is_valid(nextr, nextc, pp.first.second, &brks))
			{
				if (brks)
					q.push(make_pair(make_pair(pp.first.first + 1, 1), make_pair(nextr, nextc)));
				else
